...
export default function Home8Sec8PinEffect() {
    ...
    useEffect(() => {
        // Sections marked data-local-pin own their pin locally (e.g. CollectionPage's
        // InsightsFromPartners, which needs refresh timing decoupled from that page's
        // other pinned sections). Skip them here so we don't double-pin.
        const sec8 = document.querySelector<HTMLElement>(".sec-8-home-8:not([data-local-pin])");
        if (!sec8) return;
